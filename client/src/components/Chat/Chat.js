...
function Chat({ user, recipient, cable, setMatchChatDisplay, showUnreadMessages, setShowUnreadMessages, matchChatDisplay }) {
  ...
  return (
    <div className="chat-display" style={{ display: matchChatDisplay === 0 ? 'none' : 'block' }}>
      <div className="chat-display-header">
        <h3>{recipient.first_name}</h3>
        <div className="close-icon" onClick={() => setMatchChatDisplay(0)}>ⓧ</div>
      </div>
